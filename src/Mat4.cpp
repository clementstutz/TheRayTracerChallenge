...
Mat4& Mat4::operator*=(Mat4 const& other) {
	...
	//Strassen algorithm
	// D�composition en sous-matrices 2x2
...
void Mat4::afficher(std::ostream& flux) const {
	//flux << std::fixed << std::setprecision(2); // Fixer l'affichage des floats avec 2 chiffres apr�s la virgule
	...
	//flux << std::defaultfloat; // Restaurer le formatage par d�faut apr�s l'affichage
}
...
Mat4& Mat4::invert() {
	...
	// Pr�-calcul des termes communs pour les mineurs
...
Mat4 Mat4::RotateMatrix(double x, double y, double z) {
	// Ces rotations sont par rapport au repere globale qui rest inchang� par les rotations!!
